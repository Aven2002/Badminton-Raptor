        ...
        <div class="row">
          ...
          <div class="col-md-8">
            ...
            <div class="d-flex justify-content-end mb-3">
              <button type="button" class="btn btn-edit" @click="toggleEditMode"><i class="fa-regular fa-pen-to-square"></i> {{ editMode ? 'Cancel' : 'Edit' }}</button>
            </div>
            ...
            <form @submit.prevent="updateProfile">
              <div class="row">
                <div class="col-md-6 mb-3" v-for="(field, index) in fields" :key="index">
                  ...
                  <div v-else>
                    <p class="form-control-plaintext">{{ field.key === 'dob' ? formatDate(user[field.key]) : user[field.key] }}</p>
                  </div>
                </div>
              </div>

              <!-- Update Button Only in Edit Mode -->
              <button v-if="editMode" type="submit" class="btn btn-update">Update</button>
            </form>
          </div>
        </div>
      </div>
